...
То же правило действует для уроков черновика: `GET /courses/{courseId}/lessons`,
`GET /lessons/{lessonId}` и `GET /lessons/{lessonId}/content` отвечают `404 NOT_FOUND`
всем, кроме автора и `admin`; список `GET /lessons` содержит только уроки
опубликованных курсов.
