...
Шаблон для курсов школы AI Sprints: **«Корутины и Flow на сервере»**
(модуль `coroutines/`), **«REST-сервис на Ktor: API, база, авторизация,
тесты»** (модуль `service/`) и карьерных курсов **«Карьерная подготовка
бэкенд-разработчика»** и «Карьерное сопровождение выпускника» (папка
`career/` — код главы 4 и накопительные файлы остальных глав, подробности
в `career/README.md`). В шаблоне уже есть сборка, база, миграции,
...
| `career/` | накопительные файлы карьерных курсов; подробная карта файлов по урокам — `career/README.md` |
| `career/algorithms/` | модуль `:career` — код главы 4 курса «Карьерная подготовка бэкенд-разработчика», уроки 13–16 |
| `career/reports/` | отчёты прогона главы 4 `algorithms-NN.txt` — коммитятся в pull request |
| `.github/workflows/career.yml` | CI главы 4: номер урока из ветки `career-NN` (13–16), тесты и сверка отчёта |
...
1. На каждый урок — своя ветка от свежего `main`. Префикс у каждого курса
   свой, чтобы ветки разных курсов в одном репозитории не пересекались:

   | Курс | Ветка | Пример |
   |---|---|---|
   | «Корутины и Flow на сервере» | `lesson-NN` | `lesson-01`, `lesson-02` |
   | «REST-сервис на Ktor» | `rest-NN` | `rest-01`, `rest-02` |
   | «Карьерная подготовка бэкенд-разработчика» | `career-NN` | `career-01`, `career-13` |
   | «Карьерное сопровождение выпускника» | `career-support-NN` | `career-support-08`, `career-support-09` |

   CI корутин берёт номер урока из имени ветки и принимает только
   `lesson-NN`. CI сервиса имя ветки не читает: главу он берёт из
   последнего отчёта `service/reports/tests-0N.txt`.
2. Сделайте тесты урока зелёными и сгенерируйте отчёт прогона:
   ...
   В IDE те же задачи запускаются через Run Anything: `gradle
   :service:testReport -Pchapter=N`. Отчёт правится только прогоном: CI
   перегенерирует его и сверяет с закоммиченным. В курсе REST-сервиса отчёт
   один на главу: если прогон его не изменил, в pull request его нет, и
   это нормально.
3. Закоммитьте код и отчёт, откройте pull request из ветки урока в `main`
   своего репозитория. CI прогонит тесты; ссылку на pull request отправьте
   в задаче урока на платформе.
